...
RegulatorPID::RegulatorPID(double k, double ti, double td)
    : Kp(k), Ti(ti), Td(td), metoda(MetodaCalkowania::STALA_W_SUMIE)
{
    zresetuj();
}
...
void RegulatorPID::zresetuj() {
    ...
    poprzedni_uchyb = 0.0;
    ostatnie_P = 0.0; ostatnie_I = 0.0; ostatnie_D = 0.0;
}
...
void RegulatorPID::setMetodaCalkowania(MetodaCalkowania m) {
    ...
    if (m == MetodaCalkowania::STALA_W_SUMIE) {
        if (Ti != 0) calka_wewnetrzna = suma_uchybow * (1.0/Ti);
        else calka_wewnetrzna = 0.0;
    } else {
        if (Ti != 0) suma_uchybow = calka_wewnetrzna * Ti;
        else suma_uchybow = 0.0;
    }
    metoda = m;
}
...
void RegulatorPID::setKp(double k) { Kp = k; }
double RegulatorPID::getKp() const { return Kp; }

void RegulatorPID::setTi(double ti) { Ti = ti; }
